...
class Profile(object):
    ...
    def from_data(cls, address: int, enabled: bool, data: bytes):
        ...
        profile.report_rate = 1000 / max(1, profile._report_rate)  # type: ignore
        try:
            profile._name.index(b"\xff")  # type: ignore
        except ValueError:
            # we're not using the default name, so let's decode this
            profile.name = bytes(profile._name).decode("utf-8").rstrip("\x00")  # type: ignore
